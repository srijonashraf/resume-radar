...
const AnalysisResults = ({ analysisResults }: AnalysisResultsProps) => {
  ...
  const getRecommendationColor = (recommendation: string) => {
    switch (recommendation) {
      case "Strong Hire": return "text-green-400 bg-green-400/10 border-green-400/20";
      case "Hire": return "text-blue-400 bg-blue-400/10 border-blue-400/20";
      case "Maybe": return "text-yellow-400 bg-yellow-400/10 border-yellow-400/20";
      case "No Hire": return "text-red-400 bg-red-400/10 border-red-400/20";
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card p-6">
          ...
          <div className="text-center">
            <div className={`text-6xl font-bold mb-3 ${getScoreColor(analysisResults.overallScore, 10)}`}>
              {analysisResults.overallScore}/10
            </div>
            <div className="w-full bg-slate-700 rounded-full h-4 mb-2">
              <div
                className={`h-4 rounded-full transition-all ${getScoreBgColor(analysisResults.overallScore, 10)}`}
                style={{ width: `${(analysisResults.overallScore / 10) * 100}%` }}
              ></div>
            </div>
            <p className="text-slate-400 text-sm">
              {analysisResults.overallScore >= 8 ? "Excellent" :
               analysisResults.overallScore >= 6 ? "Good" :
               analysisResults.overallScore >= 4 ? "Fair" : "Needs Improvement"}
            </p>
          </div>
        </div>
        ...
        <div className="card p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-xl font-semibold text-white">Hiring Recommendation</h3>
          </div>
          <div className="text-center">
            <div className={`inline-flex items-center px-4 py-3 rounded-xl border text-lg font-medium ${getRecommendationColor(analysisResults.hiringRecommendation)}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="card p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-semibold text-white">Years of Experience</h3>
          </div>
          <div className="text-center">
      ...
      <div className="card p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="text-right">
            <span className={`text-2xl font-bold ${getScoreColor(analysisResults.atsCompatibility.score, 100)}`}>
        ...
        <div className="space-y-3">
          <div className="w-full bg-slate-700 rounded-full h-2">
            <div
              className={`h-2 rounded-full transition-all ${getScoreBgColor(analysisResults.atsCompatibility.score, 100)}`}
              style={{ width: `${analysisResults.atsCompatibility.score}%` }}
            ></div>
          </div>
          {analysisResults.atsCompatibility.issues.length > 0 && (
            <div>
              <h4 className="text-sm font-medium text-orange-400 mb-2">Issues to Fix:</h4>
              <ul className="space-y-1">
                {analysisResults.atsCompatibility.issues.map((issue, index) => (
                  <li key={index} className="flex items-start text-sm text-orange-200">
          ...
          {analysisResults.strengthAreas.length > 0 && (
            <div className="card p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold text-white">Strength Areas</h3>
              </div>
              <ul className="space-y-3">
                {analysisResults.strengthAreas.map((strength, index) => (
                  ...
                  >
                    ...
                    <span className="text-green-200 text-sm leading-relaxed">{strength}</span>
                  </motion.li>
                ))}
              </ul>
            </div>
          ...
          {analysisResults.redFlags.length > 0 && (
            <div className="card p-6 border-red-900/50">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold text-red-400">Red Flags</h3>
              </div>
              <ul className="space-y-3">
              ...
              <ul className="space-y-3">
                {analysisResults.keyAchievements.map((achievement, index) => (
                  ...
                  >
                    ...
                    <span className="text-yellow-200 text-sm leading-relaxed">{achievement}</span>
                  </motion.li>
                ))}
              </ul>
            </div>
          ...
          <div className="card p-6">
            ...
            <div className="space-y-4">
              {analysisResults.detectedSkills.technical.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-blue-400 mb-2">Technical Skills</h4>
                  <div className="flex flex-wrap gap-2">
                    {analysisResults.detectedSkills.technical.map((skill, index) => (
                      <span
                        key={index}
              ...
              {analysisResults.detectedSkills.soft.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-green-400 mb-2">Soft Skills</h4>
                  <div className="flex flex-wrap gap-2">
              ...
              <ul className="space-y-3">
                {analysisResults.improvementAreas.map((area, index) => (
                  ...
                  >
                    ...
                    <span className="text-orange-200 text-sm leading-relaxed">{area}</span>
                  </motion.li>
                ))}
              </ul>
            </div>
          ...
          <div className="card p-6">
            ...
            <div className="space-y-4">
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {analysisResults.recommendations.immediate.map((rec, index) => (
                      <li key={index} className="flex items-start text-sm text-red-200">
              ...
              {analysisResults.recommendations.shortTerm.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-yellow-400 mb-2">Short-term Goals</h4>
                  <ul className="space-y-2">
                    {analysisResults.recommendations.shortTerm.map((rec, index) => (
                      <li key={index} className="flex items-start text-sm text-yellow-200">
                        <CheckCircleIcon className="h-4 w-4 mr-2 mt-0.5 flex-shrink-0" />
                        {rec}
                      </li>
                    ))}
              ...
              {analysisResults.recommendations.longTerm.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-green-400 mb-2">Long-term Goals</h4>
                  <ul className="space-y-2">
                    {analysisResults.recommendations.longTerm.map((rec, index) => (
                      <li key={index} className="flex items-start text-sm text-green-200">
                        <CheckCircleIcon className="h-4 w-4 mr-2 mt-0.5 flex-shrink-0" />
                        {rec}
                      </li>
                    ))}
